/*
 * mat4x2.c - 4x2 matrix operations for the CFW math module.
 *
 * See mat4x2.h for API documentation and usage examples.
 */
...
void math_mat4x2_copy_1(FSize const *const mat, FSize *const dest) {
    ...
    mat4x2 cm = DEFAULT_INITIALIZATION;
    mat4x2 cd = DEFAULT_INITIALIZATION;

    _math_mat4x2_raw_to_cglm(mat, cm);
    glmc_mat4x2_copy(cm, cd);
...
void math_mat4x2_mul_1(FSize const *const a, FSize const *const b, FSize *const dest) {
    ...
    mat4x2 cm1 = DEFAULT_INITIALIZATION;
    mat2x4 cm2 = DEFAULT_INITIALIZATION;
    mat2 cd = DEFAULT_INITIALIZATION;

    _math_mat4x2_raw_to_cglm(a, cm1);
    _math_mat2x4_raw_to_cglm(b, cm2);
    glmc_mat4x2_mul(cm1, cm2, cd);
...
Mat2 math_mat4x2_mul_2(Mat4x2 const a, Mat2x4 const b) {
    ...
    _math_mat4x2_to_cglm(a, cm1);
    _math_mat2x4_to_cglm(b, cm2);
    glmc_mat4x2_mul(cm1, cm2, cd);
...
void math_mat4x2_mulv_1(FSize const *const m, FSize const *const v, FSize *const dest) {
    ...
    _math_mat4x2_raw_to_cglm(m, cm);
    _math_vec4_raw_to_cglm(v, cv);
    glmc_mat4x2_mulv(cm, cv, cd);
...
void math_mat4x2_scale_1(FSize const *const mat, FSize const s, FSize *const dest) {
    ...
    _math_mat4x2_raw_to_cglm(mat, cm);
    glmc_mat4x2_scale(cm, (float) s);
    _math_mat4x2_raw_from_cglm(cm, dest);
...
Mat4x2 math_mat4x2_scale_2(Mat4x2 const mat, FSize const s) {
    ...
    Mat4x2 const result = _math_mat4x2_from_cglm(cm);
