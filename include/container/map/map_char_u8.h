/*
 * map_char_u8.h - Map from char* to U8 (string-to-scalar map) for the C Libraries Framework
 *
 * Features:
 *   - Map from string keys to U8 values, over parallel AL_Char and AL_U8 lists
 ...
 *   Map_Char_U8 counts = map_char_u8_init_1();
 *
 *   map_char_u8_add(&counts, char_new_2("retries"), 3);
 *
 *   U8 *const retries = map_char_u8_at_1(&counts, "retries");
 *
 *   map_char_u8_uninit(&counts);
 *
 *   // For a key you do NOT want to give up - a literal, a stack buffer - add_static copies it.
 *   Map_Char_U8 limits = map_char_u8_init_1();
 *
 *   map_char_u8_add_static(&limits, "timeout", 250);
 *
 *   map_char_u8_uninit(&limits);
 *   @endcode
 *
 * Error Handling:
 ...
 *       delete;
 *     - an INDEX at or past map_char_u8_get_size, on get_key, get_value and remove_at;
 ...
 *         1. Growing a list goes through al_char_reserve / al_u8_reserve, so an
 ...
 *            al_u8_init_2 and their alloc_ twins, which borrow directly rather than
 ...
 *         3. map_char_u8_shrink forwards to al_char_shrink / al_u8_shrink, which
 ...
 *
 * Memory Management:
 *   - THE MAP OWNS THE KEYS YOU ADD, AND ONLY THE KEYS. map_char_u8_add takes the
 ...
 *     map never compares values. map_char_u8_contains_1 exists precisely for that: it is
 ...
 *     unterminated.
 *   - map_char_u8_init_3 DEEP-COPIES the keys and copies the values. The caller keeps
 *     both lists and every string in them, and releases them normally - al_char_uninit
 *     and al_u8_uninit on each is correct.
 ...
 *     pair's value; clear and uninit free it outright. Treat a U8* from this map as dead
 ...
 *     rules are lifted here - they apply to both halves, and a write through a U8*
 ...
 *     the value lives in the array and a U8 cannot outgrow its slot. This is not a
 ...
 *   - <container/arrayList/al_u8.h> - the value half, which owns nothing.
 ...
 *
 * See map_char_u8.c for implementation details.
 */
#ifndef CONTAINER_MAP_CHAR_U8_H
#define CONTAINER_MAP_CHAR_U8_H
...
/**
 * @brief Map from string keys to U8 values.
 *
...
typedef struct {
    ...
    AL_U8 value; /**< Values, at matching indices with `key`. */
} Map_Char_U8;
 ...
 * @return Initialized Map_Char_U8.
 */
 ...
 * @param allocator Arena pointer. Must not be nullptr.
 * @return New map, or nullptr when the allocator declined. Free with map_char_u8_delete.
 * @note A nullptr must NOT be handed to map_char_u8_delete, which treats a null *self as
 ...
 * @param self Map pointer. Must not be nullptr.
 * @note Keeps the allocated capacity; see map_char_u8_shrink to release it.
 */
...
/**
 * @brief Release a map allocated by map_char_u8_new_* or map_char_u8_alloc_new_*.
 ...
 * @param self Map pointer. Must not be nullptr.
 * @param index Index. Must be below map_char_u8_get_size.
 ...
 *       something found by iterating. remove_1(map_char_u8_get_key(map, 3)) removes the
