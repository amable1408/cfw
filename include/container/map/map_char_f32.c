#include <container/map/map_char_f32.h>
 ...
 */
static bool _map_char_f32_append(Map_Char_F32 *const self, char *const key, F32 const value) {
    ...
    USize const value_before = al_f32_get_size(&self->value);

    al_char_add_last(&self->key, key);
    al_f32_add_last(&self->value, value);
    ...
    if (al_char_get_size(&self->key) != key_before + 1 || al_f32_get_size(&self->value) != value_before + 1) {
         ...
         *
         * The VALUE side needs no such care - al_f32_remove releases nothing, because a
        ...
        if (al_f32_get_size(&self->value) == value_before + 1) {
            al_f32_remove(&self->value, value_before);
        }
 ...
 *       map_char_f32_add_static_2 store a key that is a slice of a larger buffer.
 ...
 */
static char* _map_char_f32_copy(AL_Char const *const list, char const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);
 ...
 * @note Takes the list for the same reason _map_char_f32_copy does; the two must always be
 *       called with the same list for one pointer.
 */
static void _map_char_f32_release(AL_Char const *const list, char *const data) {
    trace_log_push(LOG_METADATA);
 ...
 */
static bool _map_char_f32_fill(Map_Char_F32 *const self, AL_Char const *const keys, AL_F32 const *const values, USize const size) {
    trace_log_push(LOG_METADATA);
    ...
    for (USize i = 0; i < size; i += 1) {
        ...
        if (!memory_empty((void*) key)) {
            key_copy = _map_char_f32_copy(&self->key, key, char_length(key));
        ...
        if (!_map_char_f32_append(self, key_copy, *al_f32_at(values, i))) {
            _map_char_f32_release(&self->key, key_copy);
 ...
 */
static bool _map_char_f32_index(Map_Char_F32 const *const self, char const *const key, USize const key_size, USize *const index) {
    trace_log_push(LOG_METADATA);
    ...
    USize const size = map_char_f32_get_size(self);
...
Map_Char_F32 map_char_f32_alloc_init_1(Arena *allocator) {
    ...
    Map_Char_F32 map_char_f32 = DEFAULT_INITIALIZATION;

    map_char_f32.key = al_char_alloc_init_1(allocator);
    map_char_f32.value = al_f32_alloc_init_1(allocator);
...
Map_Char_F32 map_char_f32_alloc_init_2(USize const capacity, Arena *allocator) {
    ...
    map_char_f32.key = al_char_alloc_init_2(capacity, allocator);
    map_char_f32.value = al_f32_alloc_init_2(capacity, allocator);
...
Map_Char_F32 map_char_f32_alloc_init_3(AL_Char const *const keys, AL_F32 const *const values, Arena *allocator) {
    ...
    USize const value_size = al_f32_get_size(values);
    ...
    if (size == 0) {
        Map_Char_F32 const map_char_f32 = map_char_f32_alloc_init_1(allocator);
    ...
    Map_Char_F32 map_char_f32 = map_char_f32_alloc_init_2(size, allocator);
    ...
    if (!_map_char_f32_fill(&map_char_f32, keys, values, size)) {
        map_char_f32_uninit(&map_char_f32);

        map_char_f32 = map_char_f32_alloc_init_1(allocator);
    }
...
Map_Char_F32* map_char_f32_alloc_new_1(Arena *allocator) {
     ...
     * al_char_reserve / al_f32_reserve, so an exhausted arena or a failing heap can still
    ...
    Map_Char_F32 *const map_char_f32 = (Map_Char_F32*) allocator_try_borrow(sizeof(Map_Char_F32), allocator);

    if (memory_empty((void*) map_char_f32)) {
        trace_log_pop();
...
Map_Char_F32* map_char_f32_alloc_new_2(USize const capacity, Arena *allocator) {
    ...
    *map_char_f32 = map_char_f32_alloc_init_2(capacity, allocator);
...
Map_Char_F32* map_char_f32_alloc_new_3(AL_Char const *const keys, AL_F32 const *const values, Arena *allocator) {
    ...
    *map_char_f32 = map_char_f32_alloc_init_3(keys, values, allocator);
...
F32* map_char_f32_at_1(Map_Char_F32 const *const self, char const *const key) {
    ...
    F32 *const value = map_char_f32_at_2(self, key, char_length(key));
...
F32* map_char_f32_at_2(Map_Char_F32 const *const self, char const *const key, USize const key_size) {
    ...
    if (!_map_char_f32_index(self, key, key_size, &index)) {
        trace_log_pop();

        return nullptr;
    }

    F32 *const value = al_f32_at(&self->value, index);
...
bool map_char_f32_contains_1(Map_Char_F32 const *const self, char const *const key) {
    ...
    bool const found = map_char_f32_contains_2(self, key, char_length(key));
...
bool map_char_f32_contains_2(Map_Char_F32 const *const self, char const *const key, USize const key_size) {
    ...
    USize index = 0;
    bool const found = _map_char_f32_index(self, key, key_size, &index);
...
bool map_char_f32_empty(Map_Char_F32 const *const self) {
    ...
    bool const empty = map_char_f32_get_size(self) == 0;
...
USize map_char_f32_get_capacity(Map_Char_F32 const *const self) {
    ...
    USize const value_capacity = al_f32_get_capacity(&self->value);
...
USize map_char_f32_get_size(Map_Char_F32 const *const self) {
    ...
    USize const value_size = al_f32_get_size(&self->value);
...
Map_Char_F32 map_char_f32_init_1(void) {
    ...
    map_char_f32.key = al_char_init_1();
    map_char_f32.value = al_f32_init_1();
...
Map_Char_F32 map_char_f32_init_2(USize const capacity) {
    ...
    map_char_f32.key = al_char_init_2(capacity);
    map_char_f32.value = al_f32_init_2(capacity);
...
Map_Char_F32 map_char_f32_init_3(AL_Char const *const keys, AL_F32 const *const values) {
    ...
    /* See map_char_f32_alloc_init_3: the shorter of the two, and an empty source builds an
     * empty map rather than tripping al_char_init_2's zero-capacity contract. */
    if (size == 0) {
        Map_Char_F32 const map_char_f32 = map_char_f32_init_1();
    ...
    Map_Char_F32 map_char_f32 = map_char_f32_init_2(size);

    if (!_map_char_f32_fill(&map_char_f32, keys, values, size)) {
        map_char_f32_uninit(&map_char_f32);

        map_char_f32 = map_char_f32_init_1();
    }
...
#ifdef ARENA_IMPLEMENTATION
    Map_Char_F32 *const map_char_f32 = (Map_Char_F32*) allocator_try_borrow(sizeof(Map_Char_F32), nullptr);
#else
    Map_Char_F32 *const map_char_f32 = (Map_Char_F32*) allocator_try_borrow(sizeof(Map_Char_F32));
#endif // ARENA_IMPLEMENTATION
...
Map_Char_F32* map_char_f32_new_2(USize const capacity) {
    ...
    *map_char_f32 = map_char_f32_init_2(capacity);
...
Map_Char_F32* map_char_f32_new_3(AL_Char const *const keys, AL_F32 const *const values) {
    ...
    *map_char_f32 = map_char_f32_init_3(keys, values);
...
bool map_char_f32_add(Map_Char_F32 *const self, char *const key, F32 const value) {
    ...
    bool const added = _map_char_f32_append(self, key, value);
...
bool map_char_f32_add_static(Map_Char_F32 *const self, char const *const key, F32 const value) {
    ...
    bool const added = map_char_f32_add_static_2(self, key, char_length(key), value);
...
bool map_char_f32_add_static_2(Map_Char_F32 *const self, char const *const key, USize const key_size, F32 const value) {
    ...
    char *const key_copy = _map_char_f32_copy(&self->key, key, key_size);
    ...
    if (!_map_char_f32_append(self, key_copy, value)) {
        _map_char_f32_release(&self->key, key_copy);
...
bool map_char_f32_remove_1(Map_Char_F32 *const self, char const *const key) {
    ...
    bool const removed = map_char_f32_remove_2(self, key, char_length(key));
...
bool map_char_f32_remove_2(Map_Char_F32 *const self, char const *const key, USize const key_size) {
     ...
     * _map_char_f32_append: a stored key belongs to the map, an unrolled append never did.
     * The index came from get_size, the smaller of the two, so it is in bounds for both
     * lists. */
    al_char_remove(&self->key, index);
    al_f32_remove(&self->value, index);
...
void map_char_f32_reserve(Map_Char_F32 *const self, USize const capacity) {
    ...
    al_f32_reserve(&self->value, capacity);
