#include <container/map/map_char_u8.h>
 ...
 */
static bool _map_char_u8_append(Map_Char_U8 *const self, char *const key, U8 const value) {
    ...
    USize const value_before = al_u8_get_size(&self->value);

    al_char_add_last(&self->key, key);
    al_u8_add_last(&self->value, value);
    ...
    if (al_char_get_size(&self->key) != key_before + 1 || al_u8_get_size(&self->value) != value_before + 1) {
         ...
         *
         * The VALUE side needs no such care - al_u8_remove releases nothing, because a
        ...
        if (al_u8_get_size(&self->value) == value_before + 1) {
            al_u8_remove(&self->value, value_before);
        }
 ...
 *       map_char_u8_add_static_2 store a key that is a slice of a larger buffer.
 ...
 */
static char* _map_char_u8_copy(AL_Char const *const list, char const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);
 ...
 * @note Takes the list for the same reason _map_char_u8_copy does; the two must always be
 *       called with the same list for one pointer.
 */
static void _map_char_u8_release(AL_Char const *const list, char *const data) {
    trace_log_push(LOG_METADATA);
 ...
 */
static bool _map_char_u8_fill(Map_Char_U8 *const self, AL_Char const *const keys, AL_U8 const *const values, USize const size) {
    trace_log_push(LOG_METADATA);
    ...
    for (USize i = 0; i < size; i += 1) {
        ...
        if (!memory_empty((void*) key)) {
            key_copy = _map_char_u8_copy(&self->key, key, char_length(key));
        ...
        if (!_map_char_u8_append(self, key_copy, *al_u8_at(values, i))) {
            _map_char_u8_release(&self->key, key_copy);
 ...
 */
static bool _map_char_u8_index(Map_Char_U8 const *const self, char const *const key, USize const key_size, USize *const index) {
    trace_log_push(LOG_METADATA);
    ...
    USize const size = map_char_u8_get_size(self);
...
#ifdef ARENA_IMPLEMENTATION
Map_Char_U8 map_char_u8_alloc_init_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "allocator", (void*) allocator);

    Map_Char_U8 map_char_u8 = DEFAULT_INITIALIZATION;

    map_char_u8.key = al_char_alloc_init_1(allocator);
    map_char_u8.value = al_u8_alloc_init_1(allocator);
...
Map_Char_U8 map_char_u8_alloc_init_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    map_char_u8.key = al_char_alloc_init_2(capacity, allocator);
    map_char_u8.value = al_u8_alloc_init_2(capacity, allocator);
...
Map_Char_U8 map_char_u8_alloc_init_3(AL_Char const *const keys, AL_U8 const *const values, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    USize const value_size = al_u8_get_size(values);
    ...
    if (size == 0) {
        Map_Char_U8 const map_char_u8 = map_char_u8_alloc_init_1(allocator);
    ...
    Map_Char_U8 map_char_u8 = map_char_u8_alloc_init_2(size, allocator);
    ...
    if (!_map_char_u8_fill(&map_char_u8, keys, values, size)) {
        map_char_u8_uninit(&map_char_u8);

        map_char_u8 = map_char_u8_alloc_init_1(allocator);
    }
...
Map_Char_U8* map_char_u8_alloc_new_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);
     ...
     * al_char_reserve / al_u8_reserve, so an exhausted arena or a failing heap can still
    ...
    Map_Char_U8 *const map_char_u8 = (Map_Char_U8*) allocator_try_borrow(sizeof(Map_Char_U8), allocator);

    if (memory_empty((void*) map_char_u8)) {
        trace_log_pop();
...
Map_Char_U8* map_char_u8_alloc_new_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    *map_char_u8 = map_char_u8_alloc_init_2(capacity, allocator);
...
Map_Char_U8* map_char_u8_alloc_new_3(AL_Char const *const keys, AL_U8 const *const values, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    *map_char_u8 = map_char_u8_alloc_init_3(keys, values, allocator);
...
U8* map_char_u8_at_1(Map_Char_U8 const *const self, char const *const key) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "key", (void*) key);

    U8 *const value = map_char_u8_at_2(self, key, char_length(key));
...
U8* map_char_u8_at_2(Map_Char_U8 const *const self, char const *const key, USize const key_size) {
    trace_log_push(LOG_METADATA);
    ...
    if (!_map_char_u8_index(self, key, key_size, &index)) {
        trace_log_pop();

        return nullptr;
    }

    U8 *const value = al_u8_at(&self->value, index);
...
bool map_char_u8_contains_1(Map_Char_U8 const *const self, char const *const key) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "key", (void*) key);

    bool const found = map_char_u8_contains_2(self, key, char_length(key));
...
bool map_char_u8_contains_2(Map_Char_U8 const *const self, char const *const key, USize const key_size) {
    trace_log_push(LOG_METADATA);
    ...
    USize index = 0;
    bool const found = _map_char_u8_index(self, key, key_size, &index);
...
bool map_char_u8_empty(Map_Char_U8 const *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    bool const empty = map_char_u8_get_size(self) == 0;
...
USize map_char_u8_get_capacity(Map_Char_U8 const *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    USize const key_capacity = al_char_get_capacity(&self->key);
    USize const value_capacity = al_u8_get_capacity(&self->value);
...
char* map_char_u8_get_key(Map_Char_U8 const *const self, USize const index) {
    trace_log_push(LOG_METADATA);
...
AL_Char* map_char_u8_get_keys(Map_Char_U8 *const self) {
    trace_log_push(LOG_METADATA);
...
USize map_char_u8_get_size(Map_Char_U8 const *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    USize const key_size = al_char_get_size(&self->key);
    USize const value_size = al_u8_get_size(&self->value);
...
U8* map_char_u8_get_value(Map_Char_U8 const *const self, USize const index) {
    trace_log_push(LOG_METADATA);
...
AL_U8* map_char_u8_get_values(Map_Char_U8 *const self) {
    trace_log_push(LOG_METADATA);
...
Map_Char_U8 map_char_u8_init_1(void) {
    trace_log_push(LOG_METADATA);
    ...
    map_char_u8.key = al_char_init_1();
    map_char_u8.value = al_u8_init_1();
...
Map_Char_U8 map_char_u8_init_2(USize const capacity) {
    trace_log_push(LOG_METADATA);
    ...
    map_char_u8.key = al_char_init_2(capacity);
    map_char_u8.value = al_u8_init_2(capacity);
...
Map_Char_U8 map_char_u8_init_3(AL_Char const *const keys, AL_U8 const *const values) {
    trace_log_push(LOG_METADATA);
    ...
    /* See map_char_u8_alloc_init_3: the shorter of the two, and an empty source builds an
     * empty map rather than tripping al_char_init_2's zero-capacity contract. */
    if (size == 0) {
        Map_Char_U8 const map_char_u8 = map_char_u8_init_1();
    ...
    Map_Char_U8 map_char_u8 = map_char_u8_init_2(size);

    if (!_map_char_u8_fill(&map_char_u8, keys, values, size)) {
        map_char_u8_uninit(&map_char_u8);

        map_char_u8 = map_char_u8_init_1();
    }
...
Map_Char_U8* map_char_u8_new_1(void) {
    trace_log_push(LOG_METADATA);
...
#ifdef ARENA_IMPLEMENTATION
    Map_Char_U8 *const map_char_u8 = (Map_Char_U8*) allocator_try_borrow(sizeof(Map_Char_U8), nullptr);
#else
    Map_Char_U8 *const map_char_u8 = (Map_Char_U8*) allocator_try_borrow(sizeof(Map_Char_U8));
#endif // ARENA_IMPLEMENTATION
...
Map_Char_U8* map_char_u8_new_2(USize const capacity) {
    trace_log_push(LOG_METADATA);
    ...
    *map_char_u8 = map_char_u8_init_2(capacity);
...
Map_Char_U8* map_char_u8_new_3(AL_Char const *const keys, AL_U8 const *const values) {
    trace_log_push(LOG_METADATA);
    ...
    *map_char_u8 = map_char_u8_init_3(keys, values);
...
bool map_char_u8_add(Map_Char_U8 *const self, char *const key, U8 const value) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);
    error_check_null(LOG_METADATA, "key", (void*) key);

    bool const added = _map_char_u8_append(self, key, value);
...
bool map_char_u8_add_static(Map_Char_U8 *const self, char const *const key, U8 const value) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "key", (void*) key);

    bool const added = map_char_u8_add_static_2(self, key, char_length(key), value);
...
bool map_char_u8_add_static_2(Map_Char_U8 *const self, char const *const key, USize const key_size, U8 const value) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);
    error_check_null(LOG_METADATA, "key", (void*) key);

    char *const key_copy = _map_char_u8_copy(&self->key, key, key_size);
    ...
    if (!_map_char_u8_append(self, key_copy, value)) {
        _map_char_u8_release(&self->key, key_copy);
...
void map_char_u8_clear(Map_Char_U8 *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    al_char_clear(&self->key);
    al_u8_clear(&self->value);

    trace_log_pop();
}

void map_char_u8_delete(Map_Char_U8 **const self) {
    trace_log_push(LOG_METADATA);
...
bool map_char_u8_remove_1(Map_Char_U8 *const self, char const *const key) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "key", (void*) key);

    bool const removed = map_char_u8_remove_2(self, key, char_length(key));
...
bool map_char_u8_remove_2(Map_Char_U8 *const self, char const *const key, USize const key_size) {
    trace_log_push(LOG_METADATA);
     ...
     * _map_char_u8_append: a stored key belongs to the map, an unrolled append never did.
     * The index came from get_size, the smaller of the two, so it is in bounds for both
     * lists. */
    al_char_remove(&self->key, index);
    al_u8_remove(&self->value, index);
...
void map_char_u8_remove_at(Map_Char_U8 *const self, USize const index) {
    trace_log_push(LOG_METADATA);
...
void map_char_u8_reserve(Map_Char_U8 *const self, USize const capacity) {
    trace_log_push(LOG_METADATA);
    ...
    al_u8_reserve(&self->value, capacity);

    trace_log_pop();
}

void map_char_u8_shrink(Map_Char_U8 *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    al_char_shrink(&self->key);
    al_u8_shrink(&self->value);

    trace_log_pop();
}

void map_char_u8_uninit(Map_Char_U8 *const self) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    al_char_uninit(&self->key);
    al_u8_uninit(&self->value);
