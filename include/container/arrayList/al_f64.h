/*
 * al_f64.h - Dynamic array list of F64 for the C Libraries Framework
 *
 * Features:
 *   - Dynamic, growable array of F64 values
 *   - Arena or heap allocation support
 *   - Add, remove, access, and manage F64 elements efficiently
 *
 * Family:
 ...
 *   only in its typed zero literals (0.0) where a U64 slot would write the
 *   integer 0.
 *
 * Usage Examples:
 *   @code
 *   AL_F64 list = al_f64_init_1();
 *   al_f64_add_last(&list, 3.14);
 *   F64 *val = al_f64_at(&list, 0);
 *   al_f64_uninit(&list);
 *   @endcode
 *
 * Error Handling:
 ...
 *     free. al_f64_uninit releases the buffer. Arena-backed lists are freed by releasing the arena.
 *
 ...
 *
 * See al_f64.c for implementation details.
 */

#ifndef CONTAINER_ARRAYLIST_F64_H
#define CONTAINER_ARRAYLIST_F64_H

#include <allocator/allocator.h>

/**
 * @brief Dynamic array list of F64 values.
 */
typedef struct {
 ...
 * @param data Array of F64 values.
...
/**
 * @brief Allocate a new AL_F64 on the arena.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_F64.
 */
AL_F64* al_f64_alloc_new_1(Arena *allocator);

/**
 * @brief Allocate a new AL_F64 with capacity on the arena.
 * @param capacity Initial capacity.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_F64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
...
/**
 * @brief Allocate a new AL_F64 from data and size on the arena.
 * @param data Array of F64 values.
 * @param data_size Number of elements.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_F64.
 ...
 * @param self Address of AL_F64 pointer.
 */
...
/**
 * @brief Allocate a new AL_F64 on the heap.
 * @return Pointer to new AL_F64.
 */
AL_F64* al_f64_new_1(void);

/**
 * @brief Allocate a new AL_F64 with capacity on the heap.
 * @param capacity Initial capacity.
 * @return Pointer to new AL_F64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
 *         enough to wrap the byte size, is checked as a programming error. Never
...
/**
 * @brief Allocate a new AL_F64 from data and size on the heap.
 * @param data Array of F64 values.
 * @param data_size Number of elements.
 * @return Pointer to new AL_F64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
...
#endif // CONTAINER_ARRAYLIST_F64_H