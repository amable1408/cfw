/*
 * al_u64.h - Dynamic array list of U64 for the C Libraries Framework
 *
 * Features:
 *   - Dynamic, growable array of U64 values
 *   - Arena or heap allocation support
 *   - Add, remove, access, and manage U64 elements efficiently
 *
 * Family:
 ...
 *   file's divergence from it. This file IS the canonical instantiation -
 *   see the other files' Family paragraphs for how each one diverges from it.
 *
 * Usage Examples:
 *   @code
 *   AL_U64 list = al_u64_init_1();
 *   al_u64_add_last(&list, 3);
 *   U64 *val = al_u64_at(&list, 0);
 *   al_u64_uninit(&list);
 *   @endcode
 *
 * Error Handling:
 ...
 *     free. al_u64_uninit releases the buffer. Arena-backed lists are freed by releasing the arena.
 *
 ...
 *
 * See al_u64.c for implementation details.
 */

#ifndef CONTAINER_ARRAYLIST_U64_H
#define CONTAINER_ARRAYLIST_U64_H

#include <allocator/allocator.h>

/**
 * @brief Dynamic array list of U64 values.
 */
typedef struct {
 ...
 * @param data Array of U64 values.
...
/**
 * @brief Allocate a new AL_U64 on the arena.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_U64.
 */
AL_U64* al_u64_alloc_new_1(Arena *allocator);

/**
 * @brief Allocate a new AL_U64 with capacity on the arena.
 * @param capacity Initial capacity.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_U64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
...
/**
 * @brief Allocate a new AL_U64 from data and size on the arena.
 * @param data Array of U64 values.
 * @param data_size Number of elements.
 * @param allocator Arena pointer.
 * @return Pointer to new AL_U64.
 ...
 * @param self Address of AL_U64 pointer.
 */
...
/**
 * @brief Allocate a new AL_U64 on the heap.
 * @return Pointer to new AL_U64.
 */
AL_U64* al_u64_new_1(void);

/**
 * @brief Allocate a new AL_U64 with capacity on the heap.
 * @param capacity Initial capacity.
 * @return Pointer to new AL_U64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
 *         enough to wrap the byte size, is checked as a programming error. Never
...
/**
 * @brief Allocate a new AL_U64 from data and size on the heap.
 * @param data Array of U64 values.
 * @param data_size Number of elements.
 * @return Pointer to new AL_U64.
 * @note capacity and data_size are CALLER contracts: zero, or a value large
...
#endif // CONTAINER_ARRAYLIST_U64_H