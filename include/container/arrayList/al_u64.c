...
#define _AL_U64_GROWTH_FACTOR 2
...
static AL_U64 _al_u64_init(Arena *allocator)
#else
static AL_U64 _al_u64_init(void)
#endif // ARENA_IMPLEMENTATION
{
    AL_U64 al_u64 = DEFAULT_INITIALIZATION;

#ifdef ARENA_IMPLEMENTATION
    if (!memory_empty((void*) allocator)) {
        al_u64.allocator = allocator;
    }
#endif // ARENA_IMPLEMENTATION
...
#ifdef ARENA_IMPLEMENTATION
AL_U64 al_u64_alloc_init_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "allocator", (void*) allocator);

    AL_U64 al_u64 = _al_u64_init(allocator);
...
AL_U64 al_u64_alloc_init_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U64 al_u64 = al_u64_alloc_init_1(allocator);

    al_u64.capacity = capacity;
    ...
    if (al_u64.capacity > USIZE_MAX / sizeof(U64)) {
        al_u64.capacity = 0;
    ...
    al_u64.data = (U64*) allocator_borrow(sizeof(U64) * al_u64.capacity, allocator);
    ...
    if (memory_empty((void*) al_u64.data)) {
        al_u64.capacity = 0;
    }
...
AL_U64 al_u64_alloc_init_3(U64 const *const data, USize const data_size, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "data", (void*) data);
    error_check_non_value_uint(LOG_METADATA, "data_size", data_size);

    AL_U64 al_u64 = al_u64_alloc_init_2(data_size, allocator);
    ...
    for (USize i = 0; i < al_u64.capacity && i < data_size; i += 1) {
        al_u64.data[i] = data[i];
...
AL_U64* al_u64_alloc_new_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "allocator", (void*) allocator);

    AL_U64 *const al_u64 = (AL_U64*) allocator_borrow(sizeof(AL_U64), allocator);
    ...
    if (memory_empty((void*) al_u64)) {
        trace_log_pop();

        return nullptr;
    }

    *al_u64 = al_u64_alloc_init_1(allocator);
...
AL_U64* al_u64_alloc_new_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U64 *const al_u64 = al_u64_alloc_new_1(allocator);
    ...
    *al_u64 = al_u64_alloc_init_2(capacity, allocator);
...
AL_U64* al_u64_alloc_new_3(U64 const *const data, USize const data_size, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    *al_u64 = al_u64_alloc_init_3(data, data_size, allocator);
...
void al_u64_add(AL_U64 *const self, U64 const data, USize const index) {
    trace_log_push(LOG_METADATA);
    ...
    if (self->size == self->capacity) {
        if (self->capacity == 0) {
            al_u64_reserve(self, _AL_U64_GROWTH_FACTOR);
        }
        else {
            al_u64_reserve(self, self->capacity > USIZE_MAX / _AL_U64_GROWTH_FACTOR ? USIZE_MAX : self->capacity * _AL_U64_GROWTH_FACTOR);
        }
    }
...
void al_u64_add_first(AL_U64 *const self, U64 const data) {
    trace_log_push(LOG_METADATA);

    al_u64_add(self, data, 0);

    trace_log_pop();
}

void al_u64_add_last(AL_U64 *const self, U64 const data) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    al_u64_add(self, data, self->size);

    trace_log_pop();
}

U64* al_u64_at(AL_U64 const *const self, USize const index) {
    trace_log_push(LOG_METADATA);
    ...
    return (U64*) &(self->data[index]);
}

U64* al_u64_back(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
    ...
    return (U64*) &(self->data[self->size - 1]);
}

void al_u64_clear(AL_U64 *const self) {
    trace_log_push(LOG_METADATA);
...
void al_u64_delete(AL_U64 **const self) {
    trace_log_push(LOG_METADATA);
...
bool al_u64_empty(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
...
U64* al_u64_front(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
    ...
    return (U64*) &(self->data[0]);
}

USize al_u64_get_capacity(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
...
U64* al_u64_get_data(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
...
USize al_u64_get_size(AL_U64 const *const self) {
    trace_log_push(LOG_METADATA);
...
AL_U64 al_u64_init_1(void) {
#ifdef ARENA_IMPLEMENTATION
    return _al_u64_init(nullptr);
#else
    return _al_u64_init();
#endif // ARENA_IMPLEMENTATION
}

AL_U64 al_u64_init_2(USize const capacity) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U64 al_u64 = al_u64_init_1();
...
#ifdef ARENA_IMPLEMENTATION
    al_u64.data = (U64*) allocator_borrow(sizeof(U64) * al_u64.capacity, nullptr);
#else
    al_u64.data = (U64*) allocator_borrow(sizeof(U64) * al_u64.capacity);
#endif // ARENA_IMPLEMENTATION
...
AL_U64 al_u64_init_3(U64 const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "data", (void*) data);
    error_check_non_value_uint(LOG_METADATA, "data_size", data_size);

    AL_U64 al_u64 = al_u64_init_2(data_size);
...
AL_U64* al_u64_new_1(void) {
    trace_log_push(LOG_METADATA);

#ifdef ARENA_IMPLEMENTATION
    AL_U64 *const al_u64 = (AL_U64*) allocator_borrow(sizeof(AL_U64), nullptr);
#else
    AL_U64 *const al_u64 = (AL_U64*) allocator_borrow(sizeof(AL_U64));
#endif // ARENA_IMPLEMENTATION

    *al_u64 = al_u64_init_1();
...
AL_U64* al_u64_new_2(USize const capacity) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U64 *const al_u64 = al_u64_new_1();

    *al_u64 = al_u64_init_2(capacity);
...
AL_U64* al_u64_new_3(U64 const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);
    ...
    *al_u64 = al_u64_init_3(data, data_size);
...
void al_u64_remove(AL_U64 *const self, USize const index) {
    trace_log_push(LOG_METADATA);
...
void al_u64_remove_first(AL_U64 *const self) {
    trace_log_push(LOG_METADATA);
    ...
    al_u64_remove(self, 0);

    trace_log_pop();
}

void al_u64_remove_last(AL_U64 *const self) {
    trace_log_push(LOG_METADATA);
    ...
    al_u64_remove(self, self->size - 1);

    trace_log_pop();
}

void al_u64_reserve(AL_U64 *const self, USize const capacity) {
    trace_log_push(LOG_METADATA);
    ...
    if (capacity > self->capacity) {
        ...
        if (self->capacity > USIZE_MAX / sizeof(U64)) {
...
#ifdef ARENA_IMPLEMENTATION
        self->data = (U64*) allocator_borrow(sizeof(U64) * self->capacity, self->allocator);
#else
        self->data = (U64*) allocator_borrow(sizeof(U64) * self->capacity);
#endif // ARENA_IMPLEMENTATION
...
void al_u64_shrink(AL_U64 *const self) {
    trace_log_push(LOG_METADATA);
...
void al_u64_uninit(AL_U64 *const self) {
    trace_log_push(LOG_METADATA);
