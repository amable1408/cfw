...
#define _AL_U16_GROWTH_FACTOR 2
...
static AL_U16 _al_u16_init(Arena *allocator)
#else
static AL_U16 _al_u16_init(void)
#endif // ARENA_IMPLEMENTATION
{
    AL_U16 al_u16 = DEFAULT_INITIALIZATION;

#ifdef ARENA_IMPLEMENTATION
    if (!memory_empty((void*) allocator)) {
        al_u16.allocator = allocator;
    }
#endif // ARENA_IMPLEMENTATION
...
#ifdef ARENA_IMPLEMENTATION
AL_U16 al_u16_alloc_init_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "allocator", (void*) allocator);

    AL_U16 al_u16 = _al_u16_init(allocator);
...
AL_U16 al_u16_alloc_init_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U16 al_u16 = al_u16_alloc_init_1(allocator);

    al_u16.capacity = capacity;
    ...
    if (al_u16.capacity > USIZE_MAX / sizeof(U16)) {
        al_u16.capacity = 0;
    ...
    al_u16.data = (U16*) allocator_borrow(sizeof(U16) * al_u16.capacity, allocator);
    ...
    if (memory_empty((void*) al_u16.data)) {
        al_u16.capacity = 0;
    }
...
AL_U16 al_u16_alloc_init_3(U16 const *const data, USize const data_size, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "data", (void*) data);
    error_check_non_value_uint(LOG_METADATA, "data_size", data_size);

    AL_U16 al_u16 = al_u16_alloc_init_2(data_size, allocator);
    ...
    for (USize i = 0; i < al_u16.capacity && i < data_size; i += 1) {
        al_u16.data[i] = data[i];
...
AL_U16* al_u16_alloc_new_1(Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "allocator", (void*) allocator);

    AL_U16 *const al_u16 = (AL_U16*) allocator_borrow(sizeof(AL_U16), allocator);
    ...
    if (memory_empty((void*) al_u16)) {
        trace_log_pop();

        return nullptr;
    }

    *al_u16 = al_u16_alloc_init_1(allocator);
...
AL_U16* al_u16_alloc_new_2(USize const capacity, Arena *allocator) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U16 *const al_u16 = al_u16_alloc_new_1(allocator);
    ...
    *al_u16 = al_u16_alloc_init_2(capacity, allocator);
...
AL_U16* al_u16_alloc_new_3(U16 const *const data, USize const data_size, Arena *allocator) {
    trace_log_push(LOG_METADATA);
    ...
    *al_u16 = al_u16_alloc_init_3(data, data_size, allocator);
...
void al_u16_add(AL_U16 *const self, U16 const data, USize const index) {
    trace_log_push(LOG_METADATA);
    ...
    if (self->size == self->capacity) {
        if (self->capacity == 0) {
            al_u16_reserve(self, _AL_U16_GROWTH_FACTOR);
        }
        else {
            al_u16_reserve(self, self->capacity > USIZE_MAX / _AL_U16_GROWTH_FACTOR ? USIZE_MAX : self->capacity * _AL_U16_GROWTH_FACTOR);
        }
    }
...
void al_u16_add_first(AL_U16 *const self, U16 const data) {
    trace_log_push(LOG_METADATA);

    al_u16_add(self, data, 0);

    trace_log_pop();
}

void al_u16_add_last(AL_U16 *const self, U16 const data) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "self", (void*) self);

    al_u16_add(self, data, self->size);

    trace_log_pop();
}

U16* al_u16_at(AL_U16 const *const self, USize const index) {
    trace_log_push(LOG_METADATA);
    ...
    return (U16*) &(self->data[index]);
}

U16* al_u16_back(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
    ...
    return (U16*) &(self->data[self->size - 1]);
}

void al_u16_clear(AL_U16 *const self) {
    trace_log_push(LOG_METADATA);
...
void al_u16_delete(AL_U16 **const self) {
    trace_log_push(LOG_METADATA);
...
bool al_u16_empty(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
...
U16* al_u16_front(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
    ...
    return (U16*) &(self->data[0]);
}

USize al_u16_get_capacity(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
...
U16* al_u16_get_data(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
...
USize al_u16_get_size(AL_U16 const *const self) {
    trace_log_push(LOG_METADATA);
...
AL_U16 al_u16_init_1(void) {
#ifdef ARENA_IMPLEMENTATION
    return _al_u16_init(nullptr);
#else
    return _al_u16_init();
#endif // ARENA_IMPLEMENTATION
}

AL_U16 al_u16_init_2(USize const capacity) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U16 al_u16 = al_u16_init_1();
...
#ifdef ARENA_IMPLEMENTATION
    al_u16.data = (U16*) allocator_borrow(sizeof(U16) * al_u16.capacity, nullptr);
#else
    al_u16.data = (U16*) allocator_borrow(sizeof(U16) * al_u16.capacity);
#endif // ARENA_IMPLEMENTATION
...
AL_U16 al_u16_init_3(U16 const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);

    error_check_null(LOG_METADATA, "data", (void*) data);
    error_check_non_value_uint(LOG_METADATA, "data_size", data_size);

    AL_U16 al_u16 = al_u16_init_2(data_size);
...
AL_U16* al_u16_new_1(void) {
    trace_log_push(LOG_METADATA);

#ifdef ARENA_IMPLEMENTATION
    AL_U16 *const al_u16 = (AL_U16*) allocator_borrow(sizeof(AL_U16), nullptr);
#else
    AL_U16 *const al_u16 = (AL_U16*) allocator_borrow(sizeof(AL_U16));
#endif // ARENA_IMPLEMENTATION

    *al_u16 = al_u16_init_1();
...
AL_U16* al_u16_new_2(USize const capacity) {
    trace_log_push(LOG_METADATA);

    error_check_non_value_uint(LOG_METADATA, "capacity", capacity);

    AL_U16 *const al_u16 = al_u16_new_1();

    *al_u16 = al_u16_init_2(capacity);
...
AL_U16* al_u16_new_3(U16 const *const data, USize const data_size) {
    trace_log_push(LOG_METADATA);
    ...
    *al_u16 = al_u16_init_3(data, data_size);
...
void al_u16_remove(AL_U16 *const self, USize const index) {
    trace_log_push(LOG_METADATA);
...
void al_u16_remove_first(AL_U16 *const self) {
    trace_log_push(LOG_METADATA);
    ...
    al_u16_remove(self, 0);

    trace_log_pop();
}

void al_u16_remove_last(AL_U16 *const self) {
    trace_log_push(LOG_METADATA);
    ...
    al_u16_remove(self, self->size - 1);

    trace_log_pop();
}

void al_u16_reserve(AL_U16 *const self, USize const capacity) {
    trace_log_push(LOG_METADATA);
    ...
    if (capacity > self->capacity) {
        ...
        if (self->capacity > USIZE_MAX / sizeof(U16)) {
...
#ifdef ARENA_IMPLEMENTATION
        self->data = (U16*) allocator_borrow(sizeof(U16) * self->capacity, self->allocator);
#else
        self->data = (U16*) allocator_borrow(sizeof(U16) * self->capacity);
#endif // ARENA_IMPLEMENTATION
...
void al_u16_shrink(AL_U16 *const self) {
    trace_log_push(LOG_METADATA);
...
void al_u16_uninit(AL_U16 *const self) {
    trace_log_push(LOG_METADATA);
