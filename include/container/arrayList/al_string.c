...
#define _AL_STRING_GROWTH_FACTOR 2
...
static AL_String _al_string_init(Arena *allocator)
#else
static AL_String _al_string_init(void)
#endif // ARENA_IMPLEMENTATION
{
    AL_String al_string = DEFAULT_INITIALIZATION;

#ifdef ARENA_IMPLEMENTATION
    if (!memory_empty((void*) allocator)) {
        al_string.allocator = allocator;
    }
#endif // ARENA_IMPLEMENTATION
...
AL_String al_string_alloc_init_1(Arena *allocator) {
    ...
    AL_String al_string = _al_string_init(allocator);
...
AL_String al_string_alloc_init_2(USize const capacity, Arena *allocator) {
    ...
    AL_String al_string = al_string_alloc_init_1(allocator);

    al_string.capacity = capacity;
    ...
    if (al_string.capacity > USIZE_MAX / sizeof(String)) {
        al_string.capacity = 0;
    ...
    al_string.data = (String*) allocator_borrow(sizeof(String) * al_string.capacity, allocator);
    ...
    if (memory_empty((void*) al_string.data)) {
        al_string.capacity = 0;
    }
...
AL_String al_string_alloc_init_3(String *const data, USize const data_size, Arena *allocator) {
    ...
    AL_String al_string = al_string_alloc_init_2(data_size, allocator);
    ...
    for (USize i = 0; i < al_string.capacity && i < data_size; i += 1) {
        al_string.data[i] = data[i];
...
AL_String* al_string_alloc_new_1(Arena *allocator) {
    ...
    AL_String *const al_string = (AL_String*) allocator_borrow(sizeof(AL_String), allocator);
    ...
    if (memory_empty((void*) al_string)) {
        trace_log_pop();

        return nullptr;
    }

    *al_string = al_string_alloc_init_1(allocator);
...
AL_String* al_string_alloc_new_2(USize const capacity, Arena *allocator) {
    ...
    AL_String *const al_string = al_string_alloc_new_1(allocator);
    ...
    *al_string = al_string_alloc_init_2(capacity, allocator);
...
AL_String* al_string_alloc_new_3(String *const data, USize const data_size, Arena *allocator) {
    ...
    *al_string = al_string_alloc_init_3(data, data_size, allocator);
...
void al_string_add(AL_String *const self, String *const data, USize const index) {
    ...
    if (self->size == self->capacity) {
        if (self->capacity == 0) {
            al_string_reserve(self, _AL_STRING_GROWTH_FACTOR);
        }
        else {
            al_string_reserve(self, self->capacity > USIZE_MAX / _AL_STRING_GROWTH_FACTOR ? USIZE_MAX : self->capacity * _AL_STRING_GROWTH_FACTOR);
        }
    }
    ...
    self->data[index] = memory_empty((void*) data) ? string_init_1() : *data;
...
void al_string_add_last(AL_String *const self, String *const data) {
    ...
    al_string_add(self, data, self->size);
...
String* al_string_back(AL_String const *const self) {
    ...
    return (String*) &(self->data[self->size - 1]);
}
...
void al_string_clear(AL_String *const self) {
    ...
    /* uninit, not string_clear: clear only resets the element's SIZE and keeps
     * its buffer, so every allocation in the list leaked at end of life. The
     * replacement empty carries the element's own allocator across, matching
     * al_str_clear - this file had drifted from that template. */
    for (USize i = 0; i < self->size; i += 1) {
        string_uninit(&self->data[i]);
#ifdef ARENA_IMPLEMENTATION
        if (!memory_empty((void*) self->data[i].allocator)) {
            self->data[i] = string_alloc_init_1(self->data[i].allocator);
        }
        else {
            self->data[i] = string_init_1();
        }
#else
        self->data[i] = string_init_1();
#endif // ARENA_IMPLEMENTATION
...
void al_string_delete(AL_String **const self) {
    ...
    /* uninit, not clear: clear rebuilds every slot as a fresh empty, which is
     * wasted work on a list about to be freed, and it leaves the backing array
     * for this function to release by hand. Mirrors al_str_delete. */
    al_string_uninit(*self);
    ...
    *self = nullptr;
    ...
    /* This function pushed a trace frame and never popped it - every delete
     * leaked a frame under TRACELOG_ENABLED, which the active build defines. */
    trace_log_pop();
}
...
#ifdef ARENA_IMPLEMENTATION
    return _al_string_init(nullptr);
#else
    return _al_string_init();
#endif // ARENA_IMPLEMENTATION
}
...
AL_String al_string_init_2(USize const capacity) {
    ...
    AL_String al_string = al_string_init_1();
...
#ifdef ARENA_IMPLEMENTATION
    al_string.data = (String*) allocator_borrow(sizeof(String) * al_string.capacity, nullptr);
#else
    al_string.data = (String*) allocator_borrow(sizeof(String) * al_string.capacity);
#endif // ARENA_IMPLEMENTATION
...
AL_String al_string_init_3(String *const data, USize const data_size) {
    ...
    AL_String al_string = al_string_init_2(data_size);
...
#ifdef ARENA_IMPLEMENTATION
    AL_String *const al_string = (AL_String*) allocator_borrow(sizeof(AL_String), nullptr);
#else
    AL_String *const al_string = (AL_String*) allocator_borrow(sizeof(AL_String));
#endif // ARENA_IMPLEMENTATION

    *al_string = al_string_init_1();
...
AL_String* al_string_new_2(USize const capacity) {
    ...
    AL_String *const al_string = al_string_new_1();

    /* init_2, not _1: the capacity was accepted, error-checked, and then
     * silently discarded by calling the capacity-less constructor - a bug
     * fixed here after being caught first in al_multipart_alloc_new_2. */
    *al_string = al_string_init_2(capacity);
...
AL_String* al_string_new_3(String *const data, USize const data_size) {
    ...
    *al_string = al_string_init_3(data, data_size);
...
void al_string_remove(AL_String *const self, USize const index) {
    ...
    /* Released BEFORE the shift. Shifting first overwrites the removed element
     * (leaking its buffer) and leaves data[size - 1] a shallow duplicate of the
     * element now at size - 2, so clearing the tail reached through that alias
     * and destroyed a LIVE element's characters. Mirrors al_str_remove, whose
     * comment records the same reasoning - this file predates that fix. */
    string_uninit(&self->data[index]);
    ...
    self->data[self->size - 1] = string_init_1();
...
void al_string_remove_last(AL_String *const self) {
    ...
    al_string_remove(self, self->size - 1);
...
void al_string_reserve(AL_String *const self, USize const capacity) {
    ...
    if (capacity > self->capacity) {
        ...
        if (self->capacity > USIZE_MAX / sizeof(String)) {
...
#ifdef ARENA_IMPLEMENTATION
        /* self->allocator, not nullptr: borrowing from the heap while releasing
         * through the list's arena crossed allocators in both directions - an
         * arena-backed list regrew on the heap, then handed that heap block to
         * the arena's release. */
        self->data = (String*) allocator_borrow(sizeof(String) * self->capacity, self->allocator);
#else
        self->data = (String*) allocator_borrow(sizeof(String) * self->capacity);
#endif // ARENA_IMPLEMENTATION
...
#ifdef ARENA_IMPLEMENTATION
        /* self->allocator, not nullptr - see al_string_reserve. */
        self->data = (String*) allocator_borrow(sizeof(String) * self->capacity, self->allocator);
#else
        self->data = (String*) allocator_borrow(sizeof(String) * self->capacity);
#endif // ARENA_IMPLEMENTATION
...
void al_string_uninit(AL_String *const self) {
    ...
    /* Elements released directly rather than through clear: clear rebuilds each
     * slot as a fresh empty, which is wasted work when the backing array is
     * about to go, and the old version of clear released nothing at all. */
    for (USize i = 0; i < self->size; i += 1) {
        string_uninit(&self->data[i]);
    }
